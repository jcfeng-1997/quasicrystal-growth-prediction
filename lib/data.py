...
class CrystalImageDataset(Dataset):
    def __init__(self, npy_dir):
        ...
        for file in sorted(os.listdir(npy_dir)):
            if file.endswith('.npy'):
                arr = np.load(os.path.join(npy_dir, file)).astype(np.float32)
                arr = (arr - self.global_min) / (self.global_max - self.global_min + 1e-8)
...
class CrystalImageTimeSeriesDataset(Dataset):
    def __init__(self, npy_dir, encoder, input_len=5, pred_len=5, device='cpu', case_filter=None, global_min=None, global_max=None):
    ...
    def _extract_alpha_epsilon(self, filename):
        match = re.search(r'alpha_([-+]?\d*\.\d+|\d+)_epsilon_([-+]?\d*\.\d+|\d+)', filename)
        if match:
    ...
    def _prepare_dataset(self):
        npy_files = sorted([file for file in os.listdir(self.npy_dir) if file.endswith('.npy')])
        if self.case_filter is not None:
            npy_files = [file for file in npy_files if any(cf in file for cf in self.case_filter)]
        else:
            npy_files = npy_files
        ...
        for file in npy_files:
            ...
            arr_tensor = torch.tensor(arr).unsqueeze(1).float().to(self.device)  # (500, 1, 256, 256)
            with torch.no_grad():
            ...
            alpha_epsilon = np.repeat(alpha_epsilon[None, :], latent.shape[0], axis=0)  # (500, 2)

            latent_augmented = np.concatenate([latent, alpha_epsilon], axis=-1)  # (500, latent_dim+2)

            for i in range(len(latent_augmented) - self.input_len - self.pred_len + 1):
                input_seq = latent_augmented[i : i + self.input_len]
                output_seq = latent[i + self.input_len : i + self.input_len + self.pred_len]
...
if __name__ == "__main__":
    ...
    try:
        vae_encoder.load_state_dict(torch.load("/home/jiachenf/quasicrystal/result/beta-vae/vae_model.pth"))
    ...
    for batch_x, batch_y in loader:
        print("Batch X shape:", batch_x.shape)  # ÊäÈëÊý¾Ý
        print("Batch Y shape:", batch_y.shape)  # Ç±ÔÚ¿Õ¼äµÄ±íÊ¾ (latent codes)
        batch_x = batch_x.to(device)
        
        batch_y = batch_y.to(device)

        # Ê¹ÓÃ decoder ½âÂë batch_y
        decoded_images = vae_encoder.decoder(batch_y)  # batch_y ÒÑ¾­ÊÇ latent codes

        # ¼ÙÉè½âÂëºóµÄÍ¼ÏñÐÎ×´Îª (batch_size, channels, height, width)
        decoded_images = decoded_images.squeeze(1)  # É¾³ý channels Î¬¶È£¬µÃµ½ (batch_size, height, width)

        # ¿ÉÊÓ»¯½âÂëºóµÄÍ¼Ïñ
        for i in range(min(5, decoded_images.size(0))):  # ÏÔÊ¾Ç° 5 ÕÅÍ¼Ïñ
            plt.subplot(1, 5, i+1)
            plt.imshow(decoded_images[i].cpu().detach().numpy(), cmap='gray')
            plt.axis('off')  # ¹Ø±Õ×ø±êÖá
        plt.show()
